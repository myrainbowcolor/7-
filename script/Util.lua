...
--- @param areaId string
--- @return table
function intergerToArea(areaId)
...
function dump(...)
	...
	local arr = { ... }
	if not next(arr) then return end
	local isRecursion = false -- judge is recursion
	local length = Util:getTableLength(arr)
	if arr[length] == "EG" then
		isRecursion = true
	end
	-- Calculate table union
	local tt = arr[1]
	if length > 1 and not isRecursion then
		for i = 2, length, 1 do
			tt = Util:valueUnion(tt, arr[i])
		end
	end
	local str = (isRecursion and "" or tostring(tt) .. " = ") .. "{\n"
	for k, v in pairs(tt) do
		local strTemp = nil
		if type(v) == "table" then
			strTemp = (isRecursion and (" "):rep(8) or "") .. tostring(k) .. " = " .. dump(v, "EG")
		else
			strTemp = (isRecursion and (" "):rep(8) or "") .. tostring(k) .. " = " .. tostring(v) .. ",\n"
		end
		str = str .. strTemp
	end
	if isRecursion then
